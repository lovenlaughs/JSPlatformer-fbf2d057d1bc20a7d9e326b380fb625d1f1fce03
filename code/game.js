...
function Stick(pos, ch) {
  this.basePos = this.pos = pos.plus(new Vector(-1, -2));
  this.size = new Vector(0.5, 3);
  this.wobble = Math.random() * Math.PI * 2;
  if (ch == "d") {
    this.speed = new Vector(3, 0);
  }
}
Stick.prototype.type = "stick";
...
Pacman.prototype.act = function(step, level) {
  var newPos = this.pos.plus(this.speed.times(step));
  if (!level.obstacleAt(newPos, this.size))
    this.pos = newPos;
  else if (this.repeatPos)
...
Stick.prototype.act = function(step) {
  this.wobble += step * wobbleSpeed;
  var wobblePos = Math.sin(this.wobble) * wobbleDist;
  this.pos = this.basePos.plus(new Vector(0, wobblePos));
};
...
Level.prototype.playerTouched = function(type, actor) {
  ...
  img.src = "http://www.hey.fr/tools/emoji/ios_emoji_tired_face.png";
  document.body.appendChild(img);
  
  }
  if (type == "diamond" && this.status == null) {
  this.status = "lost";
  this.finishDelay = 1;
  ...
  if (type == "pacman" && this.status == null) {
  this.status = "lost";
  this.finishDelay = 1;
  ...
  if (type == "stick" && this.status == null) {
    this.status = "lost";
    this.finishDelay = 1;
  ...
  document.body.appendChild(img);
    ...
    this.status = "won";
      this.finishDelay = 1;
      var img = document.createElement('img');
    img.src = "https://images.rapgenius.com/d0c425c18223e25ff98d97875173379e.200x200x1.png";
    document.body.appendChild(img);
    }
  }
};
